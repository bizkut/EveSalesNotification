...
- **Instant Interactivity & Non-Blocking Backfill**: To provide an excellent user experience, the bot is designed to be interactive immediately after adding a character.
  - **Instant Menu**: When a character is first added, the bot instantly displays the main menu and sends a confirmation that the background data sync has started. This allows the user to begin using other bot features without waiting.
  - **Two-Phase Backfill**: The data sync itself is a non-blocking, two-phase process. A background job fetches the most recent data first (fast sync), followed by a gradual backfill of the complete transaction history. This process is resilient and will automatically retry if it encounters temporary ESI API errors. A final notification is sent when the full sync is complete.
